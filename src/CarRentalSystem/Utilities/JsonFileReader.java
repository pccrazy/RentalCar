...
public class JsonFileReader {
    ...
    public enum file{ Users,Cars }

    public static  <T> T jsonToPojo(file whichFile,Class<T> pojoClass) {
        try {
            Gson gson = new Gson();
            InputStream in = pojoClass.getClass().getResourceAsStream("/CarRentalSystem/JsonFiles/"+whichFile+".json");
    ...
    public static  <T> void WriteToJson(file toFile, Class<T> pojoClass){
        try {
                ...
                FileWriter writer = new FileWriter(pojoClass.getClass().getResource("/CarRentalSystem/JsonFiles/"+toFile+".json").getFile());
                writer.write(json);
                writer.close();
            } catch (IOException e) {
                e.printStackTrace();
